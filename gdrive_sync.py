    ...
    if os.path.exists(CREDENTIALS_PATH_ENC):
        ...
        while True:
            current_password = getpass.getpass(
                f"Enter password to decrypt '{CREDENTIALS_PATH_ENC}': ")
            ...
            try:
                ...
                decrypted_json_str = decrypt_content(
                    encrypted_content_with_salt, current_password)
                client_config = json.loads(decrypted_json_str)
                print("Credentials decrypted successfully.")
                break
            except Exception as e:
                print(
                    f"Failed to decrypt: {e}. Incorrect password or corrupted file?")
        ...
        try:
            ...
            encrypt_choice = input(
                f"Do you want to encrypt '{CREDENTIALS_PATH}' now? (y/n): ").lower()
            if encrypt_choice == 'y':
                new_password = ""  # Initialize to ensure it's defined
                while True:
                    new_password = getpass.getpass(
                        f"Enter a new password to encrypt '{CREDENTIALS_PATH}': ")
                    if not new_password:
                        print("Password cannot be empty. Please try again.")
                        continue
                ...
                try:
                    ...
                    encrypted_data = encrypt_content(
                        plain_content, new_password)
                    with open(CREDENTIALS_PATH_ENC, 'wb') as f_enc:
                        f_enc.write(encrypted_data)
                    print(
                        f"Successfully encrypted credentials to '{CREDENTIALS_PATH_ENC}'.")
                    print(
                        f"IMPORTANT: Please MANUALLY DELETE the original unencrypted file '{CREDENTIALS_PATH}' for security.")
                except Exception as e:
        ...
        print(
            f"Please place '{CREDENTIALS_PATH}' (downloaded from Google Cloud Console)")
        print(
            f"or an existing '{CREDENTIALS_PATH_ENC}' in the script's directory.")
        return None
...
def authenticate_gdrive():
    ...
    if os.path.exists(TOKEN_PATH):
        try:
            ...
            print(
                f"Error loading token from {TOKEN_PATH}: {e}. Will try to re-authenticate.")
        ...
        if not creds:  # This will be true if initial load failed, token expired and couldn't refresh, or no token existed
            try:
                ...
                flow = InstalledAppFlow.from_client_config(
                    client_config_dict, SCOPES)
                creds = flow.run_local_server(port=0)
            except Exception as e:
                print(f"Error during authentication flow: {e}")
                print(
                    f"Ensure your '{CREDENTIALS_PATH}' or '{CREDENTIALS_PATH_ENC}' is valid.")
                return None
...
def create_drive_folder(service, name, parent_id):
    ...
    """
    if DRY_RUN:
        # Sanitize name for placeholder ID to avoid issues if name contains
        # problematic characters
        safe_name_part = "".join(
            c if c.isalnum() or c in (
                ' ', '_', '-') else '_' for c in name)
        print(
            f"[DRY RUN] Would create folder: '{name}' in parent ID '{parent_id}'")
...
def upload_or_update_file(
    ...
    local_mtime_dt_utc = datetime.datetime.fromtimestamp(
        local_mtime_ts, datetime.timezone.utc)
    local_size = local_file_path.stat().st_size

    if existing_drive_file:
        drive_file_id = existing_drive_file['id']
        drive_mtime_str = existing_drive_file.get('modifiedTime')
        drive_size_str = existing_drive_file.get(
            'size')  # Size in Drive is string
        ...
        if drive_mtime_str:
            try:
                ...
                drive_mtime_dt_utc = datetime.datetime.fromisoformat(
                    drive_mtime_str.replace('Z', '+00:00'))
            except ValueError:  # Fallback for slightly different ISO formats if any
                # Import here to avoid making it a hard dependency if not
                # needed
                from dateutil import parser
                drive_mtime_dt_utc = parser.isoparse(drive_mtime_str)

            # Compare modification times (local is newer by more than a small
            # tolerance, e.g., 2 seconds)
            if local_mtime_dt_utc > drive_mtime_dt_utc + \
                    datetime.timedelta(seconds=2):
                print(
                    f"  Local file '{file_name}' is newer based on timestamp.")
                needs_update = True
            else:
                # If timestamps are close, check size
                if drive_size_str and int(drive_size_str) != local_size:
                    print(
                        f"  Local file '{file_name}' size mismatch (Local: {local_size}, Drive: {drive_size_str}).")
                    needs_update = True
                elif COMPARE_HASHES and drive_md5:
                    # If timestamps and sizes are same, optionally check MD5
                    # hash
                    # Renamed to avoid conflict
                    local_md5_val = get_file_md5(local_file_path)
                    if local_md5_val and local_md5_val != drive_md5:
                        print(f"  Local file '{file_name}' MD5 mismatch.")
                        needs_update = True
                    elif not local_md5_val:
                        print(
                            f"  Could not calculate local MD5 for '{file_name}', assuming update needed if hashes compared.")
                        needs_update = True
                elif COMPARE_HASHES and not drive_md5:
                    print(
                        f"  Drive file '{file_name}' has no MD5, cannot compare hashes. Assuming update needed if hashes compared.")
                    needs_update = True
        else:  # No modifiedTime on Drive file, assume update
            print(
                f"  Drive file '{file_name}' has no modifiedTime, scheduling update.")
            needs_update = True
        ...
        if DRY_RUN:
            print(
                f"  [DRY RUN] Would update file: '{file_name}' (ID: {drive_file_id})")
            return drive_file_id

        print(
            f"  Updating file: '{file_name}' (ID: {drive_file_id}) in Drive...")
        media = MediaFileUpload(
            str(local_file_path),
            resumable=True)  # Ensure path is string
        try:
            updated_file = service.files().update(fileId=drive_file_id, media_body=media,
                                                  fields='id, name, modifiedTime').execute()
            print(
                f"  Updated: '{updated_file.get('name')}', New ModTime: {updated_file.get('modifiedTime')}")
            return updated_file.get('id')
        except HttpError as error:
            print(
                f'  An API error occurred while updating "{file_name}": {error}')
            return None
    else:
        # --- Upload new file ---
        if DRY_RUN:
            # Sanitize name for placeholder ID
            safe_file_name_part = "".join(
                c if c.isalnum() or c in (
                    ' ', '_', '-', '.') else '_' for c in file_name)
            print(
                f"  [DRY RUN] Would upload new file: '{file_name}' to folder ID '{drive_folder_id}'")
            return f"dry_run_file_id_for_{safe_file_name_part.replace(' ', '_')}"

        print(
            f"  Uploading new file: '{file_name}' to Drive folder ID '{drive_folder_id}'...")
        file_metadata = {
            'name': file_name,
        ...
        try:
            file = service.files().create(body=file_metadata, media_body=media,
                                          fields='id, name, modifiedTime').execute()
            print(
                f"  Uploaded: '{file.get('name')}' (ID: {file.get('id')}), ModTime: {file.get('modifiedTime')}")
            return file.get('id')
        except HttpError as error:
            print(
                f'  An API error occurred while uploading "{file_name}": {error}')
            return None
...
def sync_directory_recursive(
    ...
    drive_items_map = {}
    # Avoid querying Drive if the parent_id is a dry_run placeholder and
    # DRY_RUN is active
    if not (DRY_RUN and drive_parent_folder_id and drive_parent_folder_id.startswith(
            "dry_run_")):
        try:
            page_token = None
            while True:
            ...
            print(
                f"Could not list items in Drive folder ID '{drive_parent_folder_id}': {error}")
            ...
            print(
                f"An unexpected error occurred listing Drive items in '{drive_parent_folder_id}': {e}")
            return
    ...
    try:
        ...
        if not local_dir_path.exists():
            print(
                f"  Local directory '{local_dir_path}' no longer exists. Skipping.")
            return
        local_items_iterator = local_dir_path.iterdir()
    except PermissionError:
        print(
            f"  Permission denied reading local directory: {local_dir_path}. Skipping.")
        return
    except FileNotFoundError:  # Should be caught by the .exists() check above, but good for safety
        print(
            f"  Local directory not found (possibly deleted during sync?): {local_dir_path}. Skipping.")
        return
    ...
    for local_item_path in local_items_iterator:
        ...
        if local_item_name.lower() in [
                'thumbs.db', '$recycle.bin', '.ds_store']:
        ...
        try:
            if local_item_path.is_file():
                ...
                if existing_drive_item and existing_drive_item[
                        'mimeType'] == 'application/vnd.google-apps.folder':
                    print(
                        f"  CONFLICT: Local file '{local_item_name}' but Drive has a FOLDER with the same name. Skipping.")
                    continue
                ...
                if existing_drive_item:
                    if existing_drive_item['mimeType'] == 'application/vnd.google-apps.folder':
                        ...
                        print(
                            f"  Local directory '{local_item_name}' found in Drive (ID: {drive_subdir_id}).")
                    else:
                        print(
                            f"  CONFLICT: Local directory '{local_item_name}' but Drive has a FILE with the same name. Skipping sync for this directory.")
                        continue
                else:
                    print(
                        f"  Local directory '{local_item_name}' not found in Drive. Creating...")
                    drive_subdir_id = create_drive_folder(
                        service, local_item_name, drive_parent_folder_id)

                if drive_subdir_id:
                    sync_directory_recursive(
                        service, local_item_path, drive_subdir_id)
                else:
                    print(
                        f"  Could not obtain Drive folder ID for '{local_item_name}'. Skipping sync for this directory.")
        except FileNotFoundError:  # If file/dir is deleted mid-process
            print(
                f"  Local item not found (possibly deleted during sync?): {local_item_path}. Skipping.")
            continue
        except PermissionError:
            print(
                f"  Permission denied accessing local item: {local_item_path}. Skipping.")
            continue
        except Exception as e:  # Catch-all for other unexpected errors for this item
            print(
                f"  An unexpected error occurred processing local item {local_item_path}: {e}. Skipping.")
            continue
...
def main():
    ...
    local_documents_path_str = input(
        f"Enter path to local directory to sync (default: '{default_docs_path_str}'): ")
    ...
    if not local_documents_path.is_dir():
        print(
            f"Error: Local directory not found or is not a directory: {local_documents_path}")
        return

    print(f"Local source directory: {local_documents_path}")
    print(
        f"Target Google Drive root backup folder: '{DRIVE_BACKUP_ROOT_FOLDER_NAME}'\n")
    ...
    drive_root_backup_folder = find_drive_item(
        service,
        DRIVE_BACKUP_ROOT_FOLDER_NAME,
        'root',
    ...
    if drive_root_backup_folder:
        ...
        print(
            f"Found root backup folder in Drive: '{DRIVE_BACKUP_ROOT_FOLDER_NAME}' (ID: {drive_root_backup_folder_id})")
    else:
        print(
            f"Root backup folder '{DRIVE_BACKUP_ROOT_FOLDER_NAME}' not found in Drive. Creating it...")
        drive_root_backup_folder_id = create_drive_folder(
            service, DRIVE_BACKUP_ROOT_FOLDER_NAME, 'root')
...
if __name__ == '__main__':
    ...
    # This is a common library, but if not present, the script might have
    # issues with some date formats.
    try:
        from dateutil import parser
    except ImportError:
        print("Warning: 'python-dateutil' library not found. `pip install python-dateutil` for more robust date parsing.")
        print(
            "The script will attempt to parse dates without it, but might be less flexible.")
    main()
